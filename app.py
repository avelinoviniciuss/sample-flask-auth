...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql:///root:admi123@127.0.1:3306/flask-crud'
...
def update_user(user_id):
    ...
    if user_id != current_user.id and current_user.role == "user":
        return make_response(jsonify({'message': "You have no permissions"}), 403)
...
def delete_user(user_id):
    ...
    if current_user.role != "admin":
        return make_response(jsonify({'message': "Operation not permitted"}), 403)
